...
func TestPhoneNumber(t *testing.T) {
	ph, err := GetPhoner().PhoneNumber(reflect.Value{}, nil)
	if err != nil {
...
func TestTollFreePhoneNumber(t *testing.T) {
	ph, err := GetPhoner().TollFreePhoneNumber(reflect.Value{}, nil)
	if err != nil {
		t.Error("Expected  not error, got err", err)
	}
...
func TestE164PhoneNumber(t *testing.T) {
	ph, err := GetPhoner().E164PhoneNumber(reflect.Value{}, nil)
	if err != nil {
		t.Error("Expected  not error, got err", err)
	}
