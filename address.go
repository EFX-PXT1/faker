...
type Addresser interface {
	Latitude(v reflect.Value, ctx *Context) (interface{}, error)
	Longitude(v reflect.Value, ctx *Context) (interface{}, error)
}
...
func (i Address) Latitude(v reflect.Value, ctx *Context) (interface{}, error) {
	kind := v.Kind()
	val := i.latitute()
...
func (i Address) Longitude(v reflect.Value, ctx *Context) (interface{}, error) {
	kind := v.Kind()
	val := i.longitude()
	if kind == reflect.Float32 {
		return val, nil
